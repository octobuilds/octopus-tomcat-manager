    ...
    try {
      const res = await fetch('/api/auth/forgot-password', {
        ...
        headers: { 
          ...
          'Accept-Language': i18n.language // Pass language to backend for email localization
        },
      ...
      if (data.success) {
        ...
        setError(data.message || 'Sıfırlama linki gönderilemedi.');
      }
    } catch (err) {
      ...
      <div style={{
        ...
        {message ? (
          <div style={{ textAlign: 'center', width: '100%' }}>
            <div style={{ padding: '1rem', background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.2)', borderRadius: '12px', color: '#10b981', marginBottom: '1.5rem' }}>
              {message}
            </div>
            <Link to="/login" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-muted)', textDecoration: 'none', transition: 'color 0.2s', fontWeight: 500 }} onMouseOver={(e) => e.currentTarget.style.color = 'var(--accent)'} onMouseOut={(e) => e.currentTarget.style.color = 'var(--text-muted)'}>
